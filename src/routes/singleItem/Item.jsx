...
export default class Item extends React.Component {
        ...
        try {
            ...
            const separateById = allProductsData.reduce(function (
                ...
                if (!singleId[singleItem.id])
                    singleId[singleItem.id] = singleItem;
                else
                    singleId[singleItem.id] = Array.isArray(
            ...
            const result = Object.keys(separateById).map(
                (e) => separateById[e]
            );
            ...
            separateProductById.forEach((cate) => {
                if (cate === uniqueId) {
                    return this.setState({
                        singleProduct: separateById[uniqueId],
                    });
                }
                if (
    ...
    render() {
        const { successMsg, handleAddProduct, handleRemoveProduct } =
            this.props;
        const { singleProduct, selectedAttributes, allAttributesAreSelected } =
            this.state;
        document.title = `${singleProduct.ItemName} | Pizza Ghar`;
        return (
